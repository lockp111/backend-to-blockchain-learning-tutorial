...
│  ────────────┼──────────────────────┼─────────────────────│
│  交易级别    │    nLockTime         │    Sequence         │
│  ────────────┼──────────────────────┼─────────────────────│
│  脚本级别    │    CLTV (OP_CLTV)    │    CSV (OP_CSV)     │
│              │                      │                     │
└─────────────────────────────────────────────────────────────┘
```
...
│  交易结构:                                                   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Version | Inputs | Outputs | nLockTime (4 bytes)    │   │
│  └─────────────────────────────────────────────────────┘   │
│                                         ▲                   │
│                                         │                   │
│                              ┌──────────┴──────────┐       │
...
│                                                             │
│  含义：                                                      │
│  • nLockTime = 0：无时间锁，可立即确认                      │
│  • nLockTime = 800000：区块高度 800000 后可确认             │
│  • nLockTime = 1704067200：2024-01-01 后可确认              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  问题: nLockTime 只阻止交易被"确认"，不阻止被"双花"        │
│                                                             │
│  场景:                                                       │
│  1. Alice 创建交易 TX_A，nLockTime = 明天                   │
│  2. Alice 把 TX_A 发给 Bob（未广播）                       │
│  3. 在 nLockTime 到期前，Alice 可以创建 TX_B 花费同一 UTXO │
│  4. TX_B 被确认，TX_A 作废                                  │
│                                                             │
│  ┌────────┐                                                 │
│  │ UTXO   │──▶ TX_A (nLockTime=明天) ──✗ 被抢先            │
│  │        │                                                 │
│  │        │──▶ TX_B (nLockTime=0) ───✓ 先被确认             │
│  └────────┘                                                 │
│                                                             │
│  解决方案: 使用 CLTV 将时间锁写入脚本                       │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  执行流程：                                                  │
│  1. Push locktime       栈: [locktime]                      │
│  2. OP_CLTV            验证时间条件，失败则交易无效         │
│  3. OP_DROP            栈: []                               │
│  4. 继续正常 P2PKH 验证...                                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  └────┴────┴────────────────────────────────────────────┘  │
│                                                             │
│  D (Disable Flag, bit 31):                                  │
│    • 1 = 禁用相对时间锁                                     │
│    • 0 = 启用相对时间锁                                     │
│                                                             │
│  T (Type Flag, bit 22):                                     │
│    • 0 = Value 表示区块数                                   │
│    • 1 = Value 表示时间 (512 秒为单位)                      │
│                                                             │
│  Value (bits 0-15):                                         │
│    • T=0: 区块数量 (最多 65535 个区块 ≈ 455 天)            │
│    • T=1: 时间单位数 (每单位 512 秒，最多 ≈ 388 天)        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  场景: 资金在被接收后，必须等待 144 个区块 (约 1 天) 才能  │
│        被花费                                               │
│                                                             │
│  锁定脚本:                                                   │
│  ┌─────────────────────────────────────────────────────┐   │
...
│  ELSE                                                       │
│      # 1 年后继承人可领取                                   │
│      <52560_blocks> OP_CLTV OP_DROP                        │
│      <heir_pubkey> OP_CHECKSIG                             │
│  ENDIF                                                      │
│                                                             │
│  操作流程：                                                  │
│  1. 每年持有者将资金移动到新地址（重置时间锁）              │
│  2. 如果持有者失去能力，1 年后继承人可取回                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
