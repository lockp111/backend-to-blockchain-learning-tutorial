...
│  • decoderawtransaction - 解码交易                          │
│  • sendrawtransaction   - 广播交易                          │
│  • gettxout             - 获取 UTXO                         │
│                                                             │
│  【钱包相关】                                                │
│  • listunspent          - 列出未花费输出                    │
│  • importaddress        - 导入监控地址                      │
...
│  │  deposits 表:                                           │ │
│  │  ┌─────────────────────────────────────────────────┐   │ │
│  │  │ id | txid | block_hash | block_height | confs   │   │ │
│  │  │    | amount | address | status | created_at     │   │ │
│  │  └─────────────────────────────────────────────────┘   │ │
│  │                                                         │ │
...
│                                                             │
│  【处理流程】                                                │
│  1. 新区块到达 → 检查是否是已知区块的子块                   │
│  2. 如果 prevHash 不匹配 → 检测到 Reorg                     │
│  3. 回滚到分叉点                                            │
│  4. 将受影响的交易标记为 orphaned                           │
│  5. 重新处理新链上的交易                                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
