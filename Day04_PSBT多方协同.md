...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Magic Bytes: 0x70736274ff ("psbt" + 0xff)                  │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Global Map (全局数据)                                 │   │
│  │   • Unsigned Transaction (未签名交易)                │   │
│  │   • Extended Public Keys (扩展公钥, 用于派生)        │   │
│  │   • Version                                          │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ Input Maps (每个输入的数据)                          │   │
│  │   • Non-Witness UTXO (完整的前序交易)               │   │
│  │   • Witness UTXO (SegWit 的前序输出)                │   │
...
│  2. Updater (更新者)                                        │
│     └── 添加签名所需的元数据                               │
│         • 添加 UTXO 信息 (Non-Witness/Witness UTXO)        │
│         • 添加 Redeem Script / Witness Script              │
│         • 添加 BIP32 派生路径                               │
│                     │                                       │
│                     ▼                                       │
│  3. Signer(s) (签名者)                                      │
│     └── 对交易进行签名                                     │
│         • 可以有多个签名者                                  │
│         • 每个签名者添加自己的 Partial Signature           │
│         • 签名者可以是硬件钱包、软件钱包、冷存储           │
│                     │                                       │
│                     ▼                                       │
│  4. Combiner (合并者)                                       │
│     └── 合并来自不同签名者的 PSBT                          │
│         • 将多个 PSBT 的签名合并到一个 PSBT               │
│         • 用于多签场景                                      │
│                     │                                       │
│                     ▼                                       │
│  5. Finalizer (终结者)                                      │
│     └── 完成所有输入的脚本                                 │
│         • 将部分签名组装成完整的 ScriptSig/Witness         │
│         • 移除中间数据，只保留最终签名                     │
│                     │                                       │
│                     ▼                                       │
│  6. Extractor (提取者)                                      │
